...
if (count($bmlist) > 1) {
...
} elseif ($post_title == "" || $post_url == "" || $post_icon) {
    $query = sprintf(
    ...
    <input type="submit" value=" OK ">
    <input type="button" value=" Cancel " onClick="self.close()">
    <?php if ($settings['show_bookmark_icon']) :
        ?><input type="submit" value="Refresh Icon" name="favicon"><?php
    endif; ?>
    ...
    $query = sprintf(
        "UPDATE bookmark SET title='%s', url='%s', description='%s', childof='%d', public='%d'
				WHERE id='%d'
				AND user='%s'",

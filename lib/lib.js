...
function bookmarkverify(bmlist) {
  if (bmlist==""){
    alert("No Bookmarks selected.");
  }
  else {
    bookmark_verify = window.open("./bookmark_verify.php?bmlist=" + bmlist, "bookmarkverify", "toolbar=no,location=no,status=no,scrollbars=yes,resizable=yes,width=500,height=450");
  }
}

function bookmarkrefresh(bmlist) {
  if (bmlist==""){
    alert("No Bookmarks selected.");
  }
  else {
    bookmark_refresh = window.open("./bookmark_refresh.php?bmlist=" + bmlist, "bookmarkrefresh", "toolbar=no,location=no,status=no,scrollbars=yes,resizable=yes,width=500,height=450");
  }
}

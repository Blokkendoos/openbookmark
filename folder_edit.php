...
if ($folderid == "" || $folderid == "0") {
    ...
    $query = sprintf(
        "SELECT name, public FROM folder WHERE id='%d' AND user='%s' AND deleted!='1'",
    ...
    <p><input type="checkbox" name="public" <?php if ($row->public) {
        echo "checked";
                                            } ?>> Public</p>
    ...
    $query = sprintf(
        "UPDATE folder SET name='%s', public='%d' WHERE id='%d' AND user='%s'",
    ...
    if ($mysql->query($query)) {
        if ($inherit) {
            ...
            if (count($tree->get_children) > 0) {
                ...
                $query = sprintf(
                    "UPDATE folder SET public='%d' WHERE id IN (%s) AND user='%s'",
                    $mysql->escape($public),
                ...
                $query = sprintf(
                    "UPDATE bookmark SET public='%d' WHERE childof IN (%s) AND user='%s'",
                    $mysql->escape($public),
                    $mysql->escape($sub_folders),
                    $mysql->escape($username)
                );
                ...
                $query = sprintf(
                    "UPDATE bookmark SET public='%d' WHERE childof='%d' AND user='%s'",
                    $mysql->escape($public),
                    $mysql->escape($folderid),
                    $mysql->escape($username)
                );

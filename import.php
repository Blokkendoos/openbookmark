    ...
    <form enctype="multipart/form-data" action="<?php echo $_SERVER['SCRIPT_NAME'];?>" method="post">
      <table border="0">
        <tr>
          ...
          <td>
            <select name="browser">
              <option value="netscape"<?php if ($default_browser == "netscape") {
                    echo " selected";
                                      } ?>>Netscape / Mozilla / IE</option>
              <option value="opera"<?php if ($default_browser == "opera") {
                    echo " selected";
                                   } ?>>Opera .adr</option>
            </select>
          </td>
...
class import
{
        ...
        while (!feof($this->fp)) {
            ...
            elseif (preg_match("/<DD>*/", $line)) {
                if (isset($insert_id)) {
                    ...
                    $query = sprintf(
                        "UPDATE bookmark SET description='%s' WHERE id='%d' and user='%s'",
    ...
    function folder_new()
    {
        ...
        $query = sprintf(
            "INSERT INTO folder (childof, name, user, public) values ('%d', '%s', '%s', '%d')",
    ...
    function bookmark_new()
    {
        ...
        $query = sprintf(
            "INSERT INTO bookmark (user, title, url, description, childof, public)
				 	values ('%s', '%s', '%s', '%s', '%d', '%d')",
            $this->mysql->escape($this->username),

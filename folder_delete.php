...
if ($folderid == "" || $folderid == 0) {
    ...
    $query = sprintf(
        "DELETE FROM folder WHERE childof IN (%s) AND user='%s'",
    ...
    $query = sprintf(
        "DELETE FROM bookmark WHERE childof IN (%s) AND user='%s'",
        $mysql->escape($folders),
        $mysql->escape($username)
    );
    if (!$mysql->query($query)) {
    ...
    $query = sprintf(
        "DELETE FROM folder WHERE id=%d AND user='%s'",
...
//-->
    ...
    if ($mysql->query($query)) {
        ...
        <p><?php echo $row->public ? $folder_opened_public : $folder_opened;
        echo " " . $row->name; ?></p>

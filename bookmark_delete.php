...
if (count($bmlist) == 0) {
...
} elseif (!$settings['confirm_delete'] || set_get_noconfirm()) {
    $bmlist = implode(",", $bmlist);
    $query = sprintf(
        "DELETE FROM bookmark WHERE id IN (%s) AND user='%s'",
        $mysql->escape($bmlist),
    ...
    $query = sprintf(
        "SELECT title, id, favicon FROM bookmark WHERE id IN (%s) AND user='%s' ORDER BY title",

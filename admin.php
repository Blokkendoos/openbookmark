...
<div style="min-width: <?php echo 230 + $settings['column_width_folder']; ?>px;">
    ...
    if ($create == 'Create') {
        if ($new_username == '' || $new_password == '') {
            ...
            $query = sprintf(
                "INSERT INTO user (username, password, admin) VALUES ('%s', md5('%s'), '%d')",
                ...
                <div style="border: 1px solid #bbb; margin: 10px; padding: 10px;">
                    ...
                    if ($delete == 'Delete') {
                        if (check_username($existing_user)) {
                            if ($noconfirm) {
                                $query = sprintf(
                                    "DELETE FROM user WHERE md5(username)=md5('%s')",
                                ...
                                $query = sprintf(
                                    "DELETE FROM bookmark WHERE md5(user)=md5('%s')",
                                    $mysql->escape($existing_user)
                                );
                                if (!$mysql->query($query)) {
                                    message($mysql->error);
                                ...
                                $query = sprintf(
                                    "DELETE FROM folder WHERE md5(user)=md5('%s')",
                                    $mysql->escape($existing_user)
                                );
                                if (!$mysql->query($query)) {
                                    message($mysql->error);
